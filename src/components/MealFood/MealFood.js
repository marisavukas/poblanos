...
export default function MealFood(props) {
  ...
  const SelectedBox = ({ onClick, isinCart }) => {
    const text = isinCart ? <div className="orderpage_food_box" onClick={() => onAddMeal(foodMeal)}>
      <div className= "orderpage_food_and_checkbox col-1"> 
        <img className="orderpage_image" src={foodMeal.image} alt={foodMeal.name}/>  
          {!inCart  &&  <MdCheckCircle fontSize={59} className="orderpage_checkmark_transparent"/>}           
      </div>
    ...
    : 
    <div className="orderpage_food_box_clicked" onClick={() => onAddMeal(foodMeal)} >
      <div className= "orderpage_food_and_checkbox col-1"> 
        <img className="orderpage_image" src={foodMeal.image} alt={foodMeal.name}/>
            {inCart  && <MdCheckCircle fontSize={59} className="orderpage_checkmark"/>}
